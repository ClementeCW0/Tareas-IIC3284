		...
		\item Para cada $j=1,\dots,m$ e $i=1,\dots,n$, definimos las variables aleatorias
			...
			\begin{enumerate}[label=\alph*)]
				\item La esperanza de las variables $X_{ji}$ viene dada por
					\[
						\E(X_{ji}) = 1\cdot\PP(X_{ji} = 1) + 0\cdot\PP(X_{ji} = 0) = \frac{1}{n}
					\]
					Por lo tanto, 
					\begin{align*}
						\E(U_i) &= \E\pa{\sum_{j=1}^mX_{ji}} \\
						        &= \sum_{j=1}^m\E(X_{ji}) \\
							&= \sum_{j=1}^m\frac{1}{n} \\
							&= \frac{m}{n}
					\end{align*}
				\item Ya que $X_{ji}^2 = X_{ji}$ en todos los valores de su recorrido ($0$ y $1$), la varianza de las variables $X_{ji}$ viene dada por
					\begin{align*}
						\Var(X_{ji}) &= \E(X_{ji}^2) - \E(X_{ji})^2\\
						             &= \E(X_{ji}) - \frac{1}{n^2} \\
							     &= \frac{1}{n} - \frac{1}{n^2} \\
							     &= \frac{n-1}{n^2}
					\end{align*}
					Por lo tanto,
					\begin{align*}
						\Var(U_i) &= \Var\pa{\sum_{j=1}^mX_{ji}} \\
						          &= \sum_{j=1}^m\Var(X_{ji}) \\
							  &= \sum_{j=1}^m\frac{n-1}{n^2} \\
							  &= \frac{m(n-1)}{n^2}
					\end{align*}
				\item Si juntamos los resultados anteriores en la desigualdad de Chebyshev, obtenemos que para todo $\lambda > 0$
					\begin{align*}
						&\PP\pa{|U_i - \E(U_i)| \geq \lambda \sqrt{\Var(U_i)}} \leq \frac{1}{\lambda^2} \\
						\iff& \PP\pa{\abs{U_i - \frac{m}{n}} \geq \lambda\frac{\sqrt{m(n-1)}}{n}} \leq \frac{1}{\lambda^2}
					\end{align*}
					En particular, si $a > 0$ y hay más de una urna, entonces $\lambda = \frac{a}{\sqrt{\Var(U_i)}} > 0$, por lo tanto
					\begin{align*}
						\PP\pa{\abs{U_i - \frac{m}{n}} \geq a} \leq \frac{m(n-1)}{a^2n^2}
					\end{align*}
